...
from .config import (
    MONGO_URI, DB_NAME, COLLECTION_NAME, MONGO_MAX_POOL_SIZE,
    MONGO_MIN_POOL_SIZE, THREAD_POOL_WORKERS, MAX_RECORDS_LIMIT,
    EMAIL_ADDRESS, EMAIL_PASSWORD, SMTP_SERVER, SMTP_PORT,
    DEVICE_EMAIL_MAP, SCHEDULE_TIME, CHART_DPI
)
...
# Simple cache for device status (cache for 5 minutes since we're fetching ALL devices)
...
async def startup_event():
    ...
    if DEVICE_EMAIL_MAP:  # Only start if devices are configured
        _scheduler_thread = threading.Thread(target=run_email_scheduler, daemon=True)
        ...
        logging.warning("⚠️ No devices configured for email reports in DEVICE_EMAIL_MAP")
...
async def battery_status_page(request: Request):
    return templates.TemplateResponse("battery_status.html", {"request": request})


@app.get("/email-scheduler", response_class=HTMLResponse)
async def email_scheduler_page(request: Request):
    return templates.TemplateResponse("email_scheduler.html", {"request": request})
...
def _generate_chart_sync(records, start_date, end_date):
    ...
    try:
        ...
        ax.bar(hourly["hour"].dt.strftime("%H:%M"), hourly["csm"], color="skyblue")

        ax.set_title(f"Hourly Consumption from {start_date} to {end_date}", fontsize=11)
        ax.set_xlabel("Hour")
        ...
        buf = io.BytesIO()
        plt.tight_layout()
        plt.savefig(buf, format="png", dpi=150, bbox_inches='tight')  # Lower DPI
        buf.seek(0)
...
async def render_chart(request: Request):
    try:
        ...
        if buf is None:
            return Response(content="Error generating chart", media_type="text/plain")

        return StreamingResponse(buf, media_type="image/png")

    except Exception as e:
        logging.error(f"Chart API error: {e}")
        return Response(content="Internal server error", media_type="text/plain")
...
def _find_duplicates_sync(device_id, start, end):
    ...
    try:
        ...
        binary_uuid = Binary.from_uuid(device_uuid, UuidRepresentation.STANDARD)
...
def _get_all_device_status_sync():
    ...
    if (_device_status_cache["data"] is not None and
        current_time - _device_status_cache["timestamp"] < CACHE_DURATION):
    ...
    try:
        ...
        all_devices_pipeline = [
            ...
            {
                "$sort": {"status": 1, "latest_time": -1}  # Active first, then by latest activity
            }
        ...
        for result in results:
            ...
            device_data = {
                ...
                "first_seen": result["first_seen"].strftime("%Y-%m-%d %H:%M:%S") if result.get("first_seen") else "Unknown"
            }

            # Set inactive start/end times
            if result["status"] == "Inactive":
                device_data["inactive_start"] = latest_time.strftime("%Y-%m-%d %H:%M")
                device_data["inactive_end"] = "Ongoing"

                # Calculate how long it's been inactive
                if hours_since_last < 24:
                    device_data["inactive_duration"] = f"{round(hours_since_last, 1)} hours"
                else:
        ...
        active_count = len([d for d in formatted_results if d["status"] == "Active"])
        inactive_count = len(formatted_results) - active_count

        logging.info(f"Successfully fetched ALL {len(formatted_results)} devices: {active_count} active, {inactive_count} inactive")
...
async def get_all_device_status():
    try:
        ...
        # Run in thread pool to avoid blocking
        loop = asyncio.get_event_loop()
        result = await loop.run_in_executor(_thread_pool, _get_all_device_status_sync)

        end_time = time.time()
        execution_time = end_time - start_time

        logging.info(f"Device status API completed in {execution_time:.2f} seconds")
...
def fetch_data_for_email(device_id):
    ...
    try:
        ...
        device_uuid = uuid.UUID(device_id)
        device_id_binary = Binary.from_uuid(device_uuid, UuidRepresentation.STANDARD)
    ...
    try:
        ...
        for record in records:
            row = {
                ...
                "battery_voltage": record.get("data", {}).get("binfo", {}).get("bvt", ""),
                "battery_power": record.get("data", {}).get("binfo", {}).get("bpon", "")
            }
            csv_data.append(row)
...
def send_email_report(to_email, device_id, chart_buf, csv_buf=None, battery_info=None):
    ...
    try:
        ...
        if csv_buf:
            try:
                ...
                csv_buf.seek(0)
                logging.info(f"📄 CSV attached for {device_id} ({len(csv_data)} chars)")
            except Exception as e:
        ...
        logging.error(f"❌ Failed to send email to {to_email} for device {device_id}: {e}")
...
def process_and_send_emails():
    ...
    for device_id, email in DEVICE_EMAIL_MAP.items():
        try:
            ...
            # Send email
            if send_email_report(email, device_id, chart, csv_data, battery_info):
                logging.info(f"✅ Report sent for {device_id} - Battery: {battery_info['status']} ({battery_info['voltage']})")
            else:
...
def run_email_scheduler():
    ...
    logging.info(f"📅 Email scheduler started - reports will be sent daily at {SCHEDULE_TIME}")
    logging.info(f"📋 Configured devices: {list(DEVICE_EMAIL_MAP.keys())}")
    ...
    try:
        if not device_id:
            ...
            if device_id not in DEVICE_EMAIL_MAP:
                raise HTTPException(status_code=404, detail="Device not found in email configuration")
            ...
            def send_single_test_email():
                ...
                return send_email_report(email, device_id, chart, csv_data, battery_info)

            loop = asyncio.get_event_loop()
            success = await loop.run_in_executor(_thread_pool, send_single_test_email)
...
def _check_single_device_status_sync(device_id: str):
    ...
    try:
        ...
        # Determine if inactive for more than 1 hour (more reasonable threshold)
        if now - last_seen > timedelta(hours=1):
            logging.info(f"[INACTIVE] Device {device_id} last seen at {last_seen}")
            return {
                "device_id": device_id,
                "status": "inactive",
                "last_seen": last_seen,
            ...
            logging.info(f"[ACTIVE] Device {device_id} last seen at {last_seen}")
            return {
                "device_id": device_id,
                "status": "active",
                "last_seen": last_seen,
...
async def check_device_status(device_id: str):
    try:
        # Run in thread pool to avoid blocking
        loop = asyncio.get_event_loop()
        result = await loop.run_in_executor(_thread_pool, _check_single_device_status_sync, device_id)
...
def _get_battery_status_sync(device_id):
    ...
    try:
        ...
        device_id_binary = Binary.from_uuid(device_id_uuid, UuidRepresentation.STANDARD)
...
async def get_battery_status(device_id: str = Query(...)):
    ...
    try:
        # Run in thread pool to avoid blocking
        loop = asyncio.get_event_loop()
        result = await loop.run_in_executor(_thread_pool, _get_battery_status_sync, device_id)
